...
class HrPayslip(models.Model):
    ...
    state_sent = fields.Selection([('sent', 'Sent'), ('not_sent', 'Not Sent')], string='State Sent',
                                  default='not_sent', index=True)
    l10n_mx_payroll_daily_salary = fields.Float(related='contract_id.l10n_mx_payroll_daily_salary',
                                                compute_sudo=True)
    l10n_mx_payroll_integrated_salary = fields.Float(related='contract_id.l10n_mx_payroll_integrated_salary',
                                                     compute_sudo=True)
...
"""
    ...
    def action_payroll_sent(self):
        ...
        if self.struct_id:
            if self.struct_id.template_id:
                template = self.struct_id.template_id
        compose_form = self.env.ref(
            'mail.email_compose_message_wizard_form', False)
        if self.company_id.l10n_mx_sent_email_bol:
            if self.company_id.l10n_mx_sent_email:
                template.email_from = "{{object.company_id.l10n_mx_sent_email}}"
        ...
        if self.struct_id.report_id:
            pdf = self.struct_id.report_id.with_context(lang=self.env.user.lang)._render_qweb_pdf([self.id])[0]
        else:
            pdf = self.env.ref('hr_payroll.action_report_payslip').sudo().with_context(
                lang=self.env.user.lang)._render_qweb_pdf([self.id])[0]
        if pdf:
        ...
        if len(attaches) > 0:
            template.write({'attachment_ids': [(6, 0, attaches)]})
            template.send_mail(self.id)
            self.write({'sent': True, 'state_sent': 'sent'})
    ...
    def _l10n_mx_edi_create_cfdi_values(self):
        ...
        if self.l10n_mx_payslip_type != 'E':
            periodicidad_pago = self.contract_id.l10n_mx_payroll_schedule_pay_id.code
        if self.struct_id.type_id:
            if self.struct_id.type_id.l10n_mx_payroll_schedule_pay_id:
                periodicidad_pago = self.struct_id.type_id.l10n_mx_payroll_schedule_pay_id.code
        if periodicidad_pago:
            values['periodicidad_pago'] = periodicidad_pago
    ...
    def _validate_l10n_mx_edi_cfdi(self, values):
        ...
        if values['employee'].address_id:
            if not values['employee'].address_id.state_id:
    ...
    def action_send_email_payroll(self):
        for slip in self.filtered(lambda p: p.state_sent == 'not_sent' and not p.sent):
            if slip.employee_id and slip.employee_id.work_email:
                ...
                if self.company_id.l10n_mx_sent_email_bol:
                    if self.company_id.l10n_mx_sent_email:
                        template_mail.email_from = "{{object.company_id.l10n_mx_sent_email}}"
...
class HrPayslipRun(models.Model):
    ...
    l10n_mx_stamped_version = fields.Selection(
        [('version_3', 'Version 3.3'),
         ('version_4', 'Version 4.0')],
        string='Stamped version',
        related='company_id.l10n_mx_stamped_version',
        compute_sudo=True)
