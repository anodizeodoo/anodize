...
class HrPayslipMassivePaymentView(models.Model):
    ...
    date_start = fields.Date('Date From',related='hr_paylip_run_id.date_start',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            # print(vals['name'])
        return super(HrPayslipMassivePaymentView, self).create(vals_list)
    ...
    def context_hr_paylip_run_id(self):
        if self.hr_paylip_run_id:
            result = self.env['hr.payslip'].search(['&',('state', 'in', ['done','verify']),('payslip_run_id', '=', self.hr_paylip_run_id.id)])
            ids_result = []
            for x in result:
                ids_result.append(x.id)
            return {'domain': {
