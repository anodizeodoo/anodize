...
PAYSLIP_TEMPLATE = 'erp_l10n_mx_hr_payroll.payroll12'
...
class PayrollHrPayslipInability(models.Model):
    _name = 'payroll.hr.payslip.inability'
    ...
    payroll_inability_type = fields.Selection(
        [('01', 'Risk of work'),
...
class HrPayslip(models.Model):
    ...
    l10n_mx_payroll_payment_date = fields.Date(
    ...
    l10n_mx_payroll_sat_status = fields.Selection(
        ...
        default='undefined')
    l10n_mx_payroll_cfdi_uuid = fields.Char(
    ...
    l10n_mx_payroll_cfdi_supplier_rfc = fields.Char(
        'Supplier RFC', copy=False, readonly=True,
        help='The supplier tax identification number.',
        compute='_compute_cfdi_values')
    l10n_mx_payroll_cfdi_customer_rfc = fields.Char(
        'Customer RFC', copy=False, readonly=True,
        help='The customer tax identification number.',
        compute='_compute_cfdi_values')
    l10n_mx_payroll_cfdi_amount = fields.Float(
        'Total Amount', copy=False, readonly=True,
        help='The total amount reported on the cfdi.',
        compute='_compute_cfdi_values')
    l10n_mx_payroll_action_title_ids = fields.One2many(
        'hr.payroll.payslip.action.titles', 'payslip_id', string='Action or Titles',
    ...
    l10n_mx_payroll_extra_node_ids = fields.One2many(
        'hr.payroll.payslip.extra.perception', 'payslip_id',
    ...
    l10n_mx_payroll_balance_favor = fields.Float(
    ...
    l10n_mx_payroll_comp_year = fields.Integer(
    ...
    l10n_mx_payroll_remaining = fields.Float(
    ...
    l10n_mx_payroll_source_resource = fields.Selection([
        ('IP', 'Own income'),
        ('IF', 'Federal income'),
    ...
    l10n_mx_payroll_amount_sncf = fields.Float(
    ...
    l10n_mx_payroll_cfdi_string = fields.Char(
        ...
        'used on report.')
    l10n_mx_payroll_cfdi_certificate_id = fields.Many2one(
    ...
    l10n_mx_payroll_origin = fields.Char(
    ...
    l10n_mx_payroll_expedition_date = fields.Date(
        string='Payslip date', readonly=True, copy=False, index=True,
        states={'draft': [('readonly', False)]},
        help="Keep empty to use the current date")
    l10n_mx_payroll_time_payslip = fields.Char(
    ...
    @api.model
    def l10n_mx_payroll_get_tfd_etree(self, cfdi):
    ...
    @api.model
    def l10n_mx_payroll_get_etree(self, cfdi):
    ...
    def _get_extra_nodes(self):
        ...
        self.l10n_mx_payroll_extra_node_ids.unlink()
        return nodes

    @api.model
    def l10n_mx_payroll_generate_cadena(self, xslt_path, cfdi_as_tree):
    ...
    def get_cfdi_related(self):
        ...
        from l10n_mx_payroll_origin, hope the next structure:
            relation type|UUIDs separated by ,"""
        # TODO - Same method that on invoice
        self.ensure_one()
        if not self.l10n_mx_payroll_origin:
            return {}
        origin = self.l10n_mx_payroll_origin.split('|')
    ...
    @api.model
    def _get_l10n_mx_payroll_cadena(self):
        self.ensure_one()
        # get the xslt path
        ...
        # return the cadena
        return self.env['account.move'].l10n_mx_payroll_generate_cadena(
            xslt_path, cfdi)
    ...
    @api.model
    def _l10n_mx_payroll_solfact_info(self, company_id, service_type):
    ...
    def _l10n_mx_edi_solfact_sign(self, pac_info):
        ...
        for record in self:
            cfdi = record.l10n_mx_payroll_cfdi
            try:
                ...
                record.l10n_mx_payroll_log_error(str(e))
                continue
            ...
            record._l10n_mx_payroll_post_sign_process(xml_signed, code, msg)
    ...
    def _l10n_mx_edi_solfact_cancel(self, pac_info):
        ...
        password = pac_info['password']
        for record in self:
            uuids = [record.l10n_mx_payroll_cfdi_uuid]
            certificate_id = record.sudo().l10n_mx_payroll_cfdi_certificate_id
    ...
    def _l10n_mx_edi_finkok_sign(self, pac_info):
        ...
        password = pac_info['password']
        for record in self:
            cfdi = base64.decodestring(record.l10n_mx_payroll_cfdi)
            try:
                transport = Transport(timeout=20)
                client = Client(url, transport=transport)
    ...
    def _l10n_mx_edi_finkok_cancel(self, pac_info):
        ...
        password = pac_info['password']
        for record in self:
            uuid = record.l10n_mx_payroll_cfdi_uuid
            certificate_id = record.sudo().l10n_mx_payroll_cfdi_certificate_id
    ...
    def _l10n_mx_payroll_call_service(self, service_type):
    ...
    def _l10n_mx_payroll_post_sign_process(self, xml_signed, code=None, msg=None):
        ...
        if xml_signed:
            ...
            self.l10n_mx_payroll_pac_status = 'signed'
            self.l10n_mx_payroll_cfdi = xml_signed
    ...
    def _l10n_mx_payroll_sign(self):
        """Call the sign service with records that can be signed.
        """
        records = self.search([
            ('l10n_mx_payroll_pac_status', 'not in',
             ['signed', 'to_cancel', 'cancelled', 'retry']),
            ('id', 'in', self.ids)])
        records._l10n_mx_payroll_call_service('sign')
    ...
    def _l10n_mx_edi_post_cancel_process(self, cancelled, code=None, msg=None):
        ...
        if cancelled:
            ...
            self.l10n_mx_payroll_pac_status = 'cancelled'
        else:
    ...
    def _l10n_mx_payroll_cancel(self):
        """Call the cancel service with records that can be signed.
        """
        records = self.search([
            ('l10n_mx_payroll_pac_status', 'in',
             ['to_sign', 'signed', 'to_cancel', 'retry']),
            ('id', 'in', self.ids)])
        for record in records:
            if record.l10n_mx_payroll_pac_status in ['to_sign', 'retry']:
                record.l10n_mx_payroll_pac_status = 'cancelled'
                record.message_post(body=_(
                    'The cancel service has been called with success'),
                    subtype='account.mt_invoice_validated')
            else:
                record.l10n_mx_payroll_pac_status = 'to_cancel'
        records = self.search([
            ('l10n_mx_payroll_pac_status', '=', 'to_cancel'),
            ('id', 'in', self.ids)])
        records._l10n_mx_payroll_call_service('cancel')
    ...
    def action_payslip_cancel(self):
        ...
        mx_payslip = self.filtered(lambda r: r.l10n_mx_payroll_is_required())
        mx_payslip._l10n_mx_payroll_cancel()
        return res
    ...
    def action_payroll_sent(self):
        ...
        template = self.env.ref(
            'erp_l10n_mx_hr_payroll.email_template_payroll_payroll', False)
    ...
    def _compute_cfdi_values(self):
        ...
        for record in self:
            ...
            record.l10n_mx_payroll_cfdi = datas
            tree = record.l10n_mx_edi_get_xml_etree(base64.decodebytes(datas))
            # if already signed, extract uuid
            tfd_node = record.l10n_mx_payroll_get_tfd_etree(tree)
            if tfd_node is not None:
                record.l10n_mx_payroll_cfdi_uuid = tfd_node.get('UUID')
            record.l10n_mx_payroll_cfdi_amount = tree.get(
                'Total', tree.get('total'))
            record.l10n_mx_payroll_cfdi_supplier_rfc = tree.Emisor.get(
                'Rfc', tree.Emisor.get('rfc'))
            record.l10n_mx_payroll_cfdi_customer_rfc = tree.Receptor.get(
                'Rfc', tree.Receptor.get('rfc'))
            record.l10n_mx_payroll_cfdi_certificate_id = self.env[
                'l10n_mx_edi.certificate'].sudo().search([
                    ('serial_number', '=', tree.get(
                        'NoCertificado', tree.get('noCertificado')))],
                    limit=1)

    def action_payroll_payslip_draft(self):
        for record in self.filtered('l10n_mx_payroll_cfdi_uuid'):
            record.l10n_mx_payroll_origin = '04|%s' % record.l10n_mx_payroll_cfdi_uuid
        self.write({
            'l10n_mx_payroll_expedition_date': False,
            'l10n_mx_payroll_time_payslip': False,
        })
        return super(HrPayslip, self).action_payroll_payslip_draft()

    def action_payslip_done(self):
        """Generates the cfdi attachments for mexican companies when validated.
        """
        result = super(HrPayslip, self).action_payslip_done()
        for record in self.filtered(lambda r: r.l10n_mx_payroll_is_required()):
            ...
            if not record.l10n_mx_payroll_expedition_date:
                record.l10n_mx_payroll_expedition_date = date_mx.date()
            if not record.l10n_mx_payroll_time_payslip:
                record.l10n_mx_payroll_time_payslip = date_mx.strftime(
                    DEFAULT_SERVER_TIME_FORMAT)
            record.l10n_mx_payroll_cfdi_name = ('%s-MX-Payroll-3-3.xml' % (
                record.number)).replace('/', '')
            record._l10n_mx_edi_retry()
            record.employee_id.employee_loan_ids.filtered(
                lambda loan: loan.payroll_payslips_count < loan.payroll_payment_term).write({
                    'payslip_ids': [(4, record.id)],
                })
        return result

    def compute_sheet(self):
        if (self.filtered(lambda r: r.l10n_mx_payroll_is_required()) and
                not self.env.user.company_id.l10n_mx_payroll_minimum_wage):
        ...
        for payslip in self:
            payslip.write({
                'l10n_mx_payroll_extra_node_ids': [
    ...
    def _l10n_mx_edi_retry(self):
        ...
        for record in self:
            ...
            if error:
                ...
                record.l10n_mx_payroll_pac_status = 'retry'
                record.message_post(body=error)
                continue
            # cfdi has been successfully generated
            record.l10n_mx_payroll_pac_status = 'to_sign'

            ctx = self.env.context.copy()
            ctx.pop('default_type', False)
            filename = (
                '%s-MX-Payroll-3-3.xml' % (record.number)).replace('/', '')
            record.l10n_mx_payroll_cfdi_name = filename
    ...
    def l10n_mx_edi_retrieve_attachments(self):
        ...
        self.ensure_one()
        if not self.l10n_mx_payroll_cfdi_name:
            return []
        domain = [
            ('res_id', '=', self.id),
            ('res_model', '=', self._name),
            ('name', '=', self.l10n_mx_payroll_cfdi_name)]
    ...
    def l10n_mx_edi_get_xml_etree(self, cfdi=None):
        ...
        self.ensure_one()
        if cfdi is None:
            cfdi = base64.decodebytes(self.l10n_mx_payroll_cfdi)
    ...
    def get_cfdi_deductions_data(self):
        categ = self.env.ref(
            'erp_l10n_mx_hr_payroll.hr_payroll_salary_rule_category_deduction_mx')
    ...
    def get_cfdi_other_payments_data(self):
        ...
        "OtrosPagos"."""
        categ = self.env.ref(
            'erp_l10n_mx_hr_payroll.hr_payroll_salary_rule_category_other_mx')
    ...
    def _l10n_mx_edi_create_cfdi(self):
        ...
        time_payslip = fields.datetime.strptime(
            self.l10n_mx_payroll_time_payslip, DEFAULT_SERVER_TIME_FORMAT).time()
        values['date'] = fields.datetime.combine(
            fields.Datetime.from_string(self.l10n_mx_payroll_expedition_date),
        ...
        cadena = self.l10n_mx_payroll_generate_cadena(CFDI_XSLT_CADENA, tree)
    ...
    def l10n_mx_edi_update_pac_status(self):
        ...
        for record in self:
            if record.l10n_mx_payroll_pac_status in ('to_sign', 'retry'):
                record._l10n_mx_edi_retry()
            elif record.l10n_mx_payroll_pac_status == 'to_cancel':
                record._l10n_mx_payroll_cancel()
    ...
    def l10n_mx_edi_update_sat_status(self):
        ...
        for record in self:
            if record.l10n_mx_payroll_pac_status not in ['signed', 'cancelled']:
                continue
            supplier_rfc = record.l10n_mx_payroll_cfdi_supplier_rfc
            customer_rfc = record.l10n_mx_payroll_cfdi_customer_rfc
            total = record.l10n_mx_payroll_cfdi_amount
            uuid = record.l10n_mx_payroll_cfdi_uuid
            params = url_encode({
                're': supplier_rfc,
                'rr': customer_rfc,
            ...
            try:
                ...
                record.l10n_mx_payroll_log_error(str(e) or e.reason.__repr__())  # noqa pragma: no cover
                continue  # pragma: no cover
            record.l10n_mx_payroll_sat_status = cfdi_sat_status.get(
...
class HrEmployee(models.Model):
    ...
    l10n_mx_payroll_syndicated = fields.Boolean(
    ...
    l10n_mx_payroll_risk_rank = fields.Many2one(
    ...
    l10n_mx_payroll_contract_regime_type = fields.Selection([
        ('02', 'Sueldos'),
        ('03', 'Jubilados'),
        ('04', 'Pensionados'),
    ...
    def get_cfdi_employee_data(self, contract):
        self.ensure_one()
        return {
            'contract_type': contract.l10n_mx_payroll_contract_type,
            'emp_syndicated': 'Sí' if self.l10n_mx_payroll_syndicated else 'No',
...
class HrPayrollPayslipOvertime(models.Model):
    _name = 'hr.payroll.payslip.overtime'
...
class HrPayrollPayslipActionTitles(models.Model):
    _name = 'hr.payroll.payslip.action.titles'
...
class HrPayrollPayslipExtraPerception(models.Model):
    _name = 'hr.payroll.payslip.extra.perception'
...
class HrPayslipRun(models.Model):
    ...
    l10n_mx_payroll_productivity_bonus = fields.Float(
    ...
    def action_payslips_done(self):
        ...
        retry_payslips = (self.slip_ids - payslips).filtered(
            lambda r: r.l10n_mx_payroll_pac_status in [
    ...
    def action_payroll_sent(self):
        ...
        for payslip in self.slip_ids.filtered(
            lambda p: (p.state == 'done' and not p.sent and
                       p.l10n_mx_payroll_pac_status == 'signed' and
