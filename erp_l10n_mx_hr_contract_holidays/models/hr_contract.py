...
class HrContract(models.Model):
    ...
    def _default_l10n_mx_number(self):
        ...
        try:
            if getattr(self.env.user.company_id.sudo(), 'l10n_mx_automatic_sequence_contracts', False):
                contract_id_sequence = getattr(self.env.user.company_id.sudo(), 'contract_id_sequence', False)
    ...
    def create(self, vals_list):
        ...
        # TODO perform this part, asigns or send sequence and state on bulk sql
        if sequence_id:
            for res in res_ids:
    ...
    def write(self, vals):
        ...
        # TODO review this part, when self is multiple
        for record in self:
