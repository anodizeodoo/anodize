...
class HRPayroll(TransactionCase):
    ...
    def setUp(self):
        ...
        self.contract.compute_payroll_integrated_salary()
        self.struct = self.env.ref(
            'erp_l10n_mx_hr_payroll.payroll_structure_data_01')
        ...
        self.env.user.company_id.write({
            'l10n_mx_payroll_minimum_wage': 80.04,
            'l10n_mx_edi_fiscal_regime': '601',
        })
        self.uid = self.env.ref('erp_l10n_mx_hr_payroll.payroll_mx_manager')
    ...
    def test_001_xml_structure(self):
        ...
        self.assertEquals(payroll.l10n_mx_payroll_pac_status, 'signed',
                          payroll.message_ids.mapped('body'))
        payroll.l10n_mx_edi_update_sat_status()
        self.assertEquals(payroll.l10n_mx_payroll_sat_status, 'not_found')
        ...
        node_payroll = payroll.l10n_mx_payroll_get_etree(xml)
        node_expected = payroll.l10n_mx_payroll_get_etree(
            self.xml_expected)
    ...
    def test_002_perception_022(self):
        ...
        self.struct = self.env.ref(
            'erp_l10n_mx_hr_payroll.payroll_structure_data_03')
        payroll = self.create_payroll()
        date = payroll.l10n_mx_payroll_payment_date - timedelta(days=380)
        self.contract.write({
            'date_start': date,
        })
        payroll.compute_sheet()
        ...
        payroll.write({
            'input_line_ids': [(0, 0, {
                ...
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_perception_039_e') # noqa
            })],
        })
        payroll.compute_sheet()
        payroll.action_payslip_done()
    ...
    def test_004_other_payment_002(self):
        ...
        self.contract.l10n_mx_payroll_infonavit_rate = 0
        payroll.compute_sheet()
        payroll.action_payslip_done()
        self.assertEquals(payroll.l10n_mx_payroll_pac_status, 'signed',
                          payroll.message_ids.mapped('body'))
        ...
        payroll.compute_sheet()
        payroll.action_payslip_done()
        self.assertEqual(payroll.l10n_mx_payroll_pac_status, 'signed',
                         payroll.message_ids.mapped('body'))
        xml = payroll.l10n_mx_edi_get_xml_etree()
        payroll = payroll.l10n_mx_payroll_get_etree(xml)
        de_107 = False
    ...
    def test_005_other_payment_004(self):
        ...
        payroll.write({
            'input_line_ids': [(0, 0, {
                ...
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_other_payment_004'), # noqa
            })],
            'l10n_mx_payroll_balance_favor': 500.0,
            'l10n_mx_payroll_comp_year': (datetime.today()).year - 1,
            'l10n_mx_payroll_remaining': 500.0,
        })
        payroll.compute_sheet()
        payroll.action_payslip_done()
        self.assertEquals(payroll.l10n_mx_payroll_pac_status, 'signed',
    ...
    def test_006_perception_045(self):
        ...
        payroll.write({
            'input_line_ids': [(0, 0, {
                ...
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_perception_045_e'), # noqa
            })],
            'l10n_mx_payroll_action_title_ids': [(0, 0, {
    ...
    def test_008_cancel_xml(self):
        ...
        payroll.action_payroll_payslip_draft()
        payroll.compute_sheet()
        payroll.action_payslip_done()
        self.assertEquals(payroll.l10n_mx_payroll_pac_status, 'signed',
                          payroll.message_ids.mapped('body'))
        ...
        payroll.action_payslip_cancel()
        self.assertTrue(
            payroll.l10n_mx_payroll_pac_status in ['cancelled', 'to_cancel'],
            payroll.message_ids.mapped('body'))
    ...
    def test_010_batches(self):
        ...
        payslip_run = self.payslip_run_obj.create({
            'name': 'Payslip VX',
            'l10n_mx_payroll_payment_date': date,
        })
        ...
        self.assertEquals(
            payslip_run.slip_ids.l10n_mx_payroll_payment_date.strftime('%Y-%m-%d'),
            date, 'Payment date not assigned in the payroll created.')
        payslip_run.action_validate()
        payslip_run.action_payslips_done()
        self.assertEquals(
            payslip_run.slip_ids.l10n_mx_payroll_pac_status, 'signed',
    ...
    def test_011_aguinaldo(self):
        ...
        self.struct = self.env.ref(
            'erp_l10n_mx_hr_payroll.payroll_structure_data_02')
        payroll = self.create_payroll()
        date = payroll.l10n_mx_payroll_payment_date - timedelta(days=380)
        self.contract.write({
            'date_start': date,
    ...
    def test_013_resign_process(self):
        ...
        self.assertEqual(payroll.l10n_mx_payroll_pac_status, 'signed',
                         payroll.message_ids.mapped('body'))
        payroll.l10n_mx_payroll_pac_status = 'retry'
        payroll.l10n_mx_edi_update_pac_status()
        for _x in range(10):
            if payroll.l10n_mx_payroll_pac_status == 'signed':
                break
            time.sleep(2)
    ...
    def test_014_assimilated(self):
        ...
        payroll = self.create_payroll()
        payroll.employee_id.l10n_mx_payroll_is_assimilated = True
        payroll.employee_id.l10n_mx_payroll_contract_regime_type = '09'
        payroll.contract_id.l10n_mx_payroll_contract_type_id.code = '99'
        payroll.compute_sheet()
        payroll.action_payslip_done()
        self.assertEqual(payroll.l10n_mx_payroll_pac_status, 'signed',
                         payroll.message_ids.mapped('body'))
    ...
    def create_payroll(self):
        return self.payslip_obj.create({
            ...
            'l10n_mx_payroll_source_resource': 'IP',
            'date_from': '%s-%s-01' % (
                time.strftime('%Y'), time.strftime('%m')),
            ...
            'input_line_ids': [(0, 0, {
                ...
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_perception_005_e'), # noqa
            }), (0, 0, {
                'amount': 300.0,
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_perception_019_g'), # noqa
            }), (0, 0, {
                'amount': 100.0,
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_deduction_006'),
            }), (0, 0, {
                'amount': 300.0,
                'contract_id': self.contract.id,
                'input_type_id': self.ref(
                    'erp_l10n_mx_hr_payroll.hr_payslip_input_type_other_payment_003'), # noqa
            })],
            'l10n_mx_payroll_inability_line_ids': [(0, 0, {
                'amount': 100.0,
                'days': 1,
                'payroll_inability_type': '02',
            })],
            'l10n_mx_payroll_overtime_line_ids': [(0, 0, {
                'amount': 300.0,
                'name': 'Overtime Test',
                'days': 1,
                'hours': 1,
